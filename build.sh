...
# Create vite config for production build
cat > client/vite.build.mjs << 'VITECONFIG'
...
cat > client/tailwind.config.cjs << 'TAILWINDCONFIG'
module.exports = {
  darkMode: ["class"],
...
cat > client/postcss.config.cjs << 'POSTCSSCONFIG'
module.exports = {
  plugins: {
    tailwindcss: {},
    autoprefixer: {},
...
# Run build
cd client && npx vite build --config vite.build.mjs
